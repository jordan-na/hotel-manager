...
const ReservationsList = ({ items, reservationType }) => {
   ...
   const deleteItemsHandler = async (itemIds) => {
      if(reservationType === "booking") {
         ...
         await rentingServices.deleteRentingsByIds(itemIds);
      }
      setModifiedItems((prevState) => {
         return prevState.filter((item) => {
            return !itemIds.includes(item.bookingId || item.rentingId);
         });
      });
   };
   ...
   if(modifiedItems.length === 0) {
      ...
      listItems = modifiedItems.map((item, i) => {
         return (
            <ReservationItem
               ...
               isEditable={reservationType === "booking"}
               index={i + 1}
            />
         );
      });
